...
std::vector<std::string> SpellSystem::GetAvailableSpells(Character* caster) const{
    ...
    {
        if (CanCast(caster, pair.first, Math::ivec2(0, 0)))
        {
...
std::string SpellSystem::GetStatusEffectDesc(const std::string& name) const
{
    auto it = status_effects_.find(name);
    return (it != status_effects_.end()) ? it->second : "";
}