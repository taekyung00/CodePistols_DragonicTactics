...
AbilityResult ShieldBash::Use(Character* user, Character* target)
{
  ...
  Engine::GetGameStateManager().GetGSComponent<EventBus>()->Publish(AbilityUsedEvent{ user, target, GetName(), damage });

  Engine::GetGameStateManager().GetGSComponent<EventBus>()->Publish(CharacterMovedEvent{
	target, targetPos, newPos,
	0 // no decrease AP
  });

  Engine::GetLogger().LogEvent(
	user->TypeName() + " uses Shield Bash on " + target->TypeName() + " for " + std::to_string(damage) + " damage and pushes them to (" + std::to_string(newPos.x) + "," + std::to_string(newPos.y) +
	")");
