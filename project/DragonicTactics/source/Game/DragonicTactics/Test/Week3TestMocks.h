...
class MockTreasureBox : public CS230::GameObject
{
  ...
  } // Placeholder until added to enum
  ...
  bool IsOpened() const
  {
	return state == TreasureBoxState::Opened || state == TreasureBoxState::Empty;
  }

  // Grid interaction
  Math::ivec2 GetGridPosition() const
  {
	return grid_pos;
  }

  void SetGridPosition(Math::ivec2 pos)
  {
	grid_pos = pos;
  }
...
class MockFireball : public MockSpellBase
{
  ...
  SpellTargetType GetTargetType() const override
  {
	return SpellTargetType::AreaOfEffect;
  }
...
class MockLavaPool : public MockSpellBase
{
  ...
  std::string GetDamagePerTurn() const
  {
	return "2d6";
  } // Damage per turn standing in lava
