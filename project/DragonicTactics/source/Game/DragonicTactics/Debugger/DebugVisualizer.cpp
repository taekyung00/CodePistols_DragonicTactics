...
void DebugVisualizer::Init()
{
  ...
  event_bus->Subscribe<CharacterDamagedEvent>([this](const CharacterDamagedEvent& e) { OnCharacterDamaged(e); });

  event_bus->Subscribe<CharacterDeathEvent>([this](const CharacterDeathEvent& e) { OnCharacterDeath(e); });

  // Subscribe to movement events
  event_bus->Subscribe<AIDecisionEvent>([this](const AIDecisionEvent& e) { OnAIDecision(e); });
...
void DebugVisualizer::Update(double dt)
{
  ...
  recent_damage_.erase(std::remove_if(recent_damage_.begin(), recent_damage_.end(), [](const DamageInfo& d) { return d.lifetime <= 0.0; }), recent_damage_.end());
	...
	for (int x = 0; x < MAP_WIDTH; ++x)
	{
	  ...
	  if (ImGui::CollapsingHeader(character->TypeName().c_str()))
	  {
		...
		if (stats != nullptr)
		{
		  ...
		  ImGui::Text("MOV: %d", stats->GetSpeed());
...
void DebugVisualizer::DrawImGuiCombatLog()
{
  ...
  for (auto it = event_log_.rbegin(); it != event_log_.rend(); ++it)
  {
	...
	{
	  ImVec4 color = entry.event_type == "Death" ? ImVec4(1.0f, 0.0f, 0.0f, 1.0f) : ImVec4(1.0f, 0.6f, 0.4f, 1.0f);
  ...
  for (const auto& dmg : recent_damage_)
  {
	...
	{
	  ImGui::Text("  %s -> %s: %d damage (%.1fs left)", dmg.attacker->TypeName().c_str(), dmg.target->TypeName().c_str(), dmg.damage, dmg.lifetime);
	}
  }
...
void DebugVisualizer::OnCharacterDamaged(const CharacterDamagedEvent& event)
{
  ...
  std::string details		= attacker_name + " dealt " + std::to_string(event.damageAmount) + " damage to " + event.target->TypeName();
...
void DebugVisualizer::OnTurnStarted(const TurnStartedEvent& event)
{
  ...
  std::string details = "Turn " + std::to_string(event.turnNumber) + ": " + event.character->TypeName() + " (" + std::to_string(event.actionPoints) + " AP)";
...
void DebugVisualizer::DrawImGuiAIDecisions()
{
  ...
  if (ImGui::BeginTable("AITable", 4, ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg | ImGuiTableFlags_Resizable))
  {
	...
	for (auto it = ai_decision_history_.rbegin(); it != ai_decision_history_.rend(); ++it)
	{
	  ...
	  if (it->actionType == "Attack")
		typeColor = ImVec4(1.0f, 0.4f, 0.4f, 1.0f); // 빨강
	  else if (it->actionType == "Move")
		typeColor = ImVec4(1.0f, 1.0f, 0.4f, 1.0f); // 노랑
	  else if (it->actionType == "Heal")
