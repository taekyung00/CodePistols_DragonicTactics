...
CS230::GameObject::GameObject(Math::vec2 _position) : GameObject(_position, 0, { 1, 1 })
{
}

CS230::GameObject::GameObject(Math::vec2 _position, double _rotation, Math::vec2 _scale)
	: current_state(&state_none), matrix_outdated(true), destroy(false), position(_position), velocity(Math::vec2{ 0.0, 0.0 }), scale(_scale), rotation(_rotation)
{
}
