...
namespace CS230
{
  ...
  bool CircleCollision::IsCollidingWith(GameObject* other_object)
  {
	...
	if (((position.x - other_position.x) * (position.x - other_position.x) + (position.y - other_position.y) * (position.y - other_position.y)) < ((_radius + other_radius) * (_radius + other_radius)))
	{
	  return true;
	}
	return false;
