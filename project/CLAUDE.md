...
- **구현 완료**: SpellSystem (CSV 파싱 + 시전 + UI), StatusEffectHandler (9가지 효과 + OnApplied/OnRemoved), TurnManager, GridSystem
...
GS 컴포넌트. 9가지 효과(`KNOWN_EFFECTS`)의 실행 로직을 타이밍별 훅으로 중앙 관리한다.
