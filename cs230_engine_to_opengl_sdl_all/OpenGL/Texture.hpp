...
namespace OpenGL
{
     ...
     *
     * The function temporarily binds the texture, updates both WRAP_S and
     * WRAP_T parameters with the new wrapping mode, then unbinds the texture.
     * Changes take effect immediately for subsequent texture sampling operations.
     */
    void SetWrapping(TextureHandle texture_handle, Wrapping wrapping) noexcept;
}
