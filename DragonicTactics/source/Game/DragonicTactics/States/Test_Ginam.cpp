...
void Test::test_json()
{
    ...
    int dragonslot  = registry.GetValue<int>("Dragon.spell_slots.1", 0);

    Engine::GetLogger().LogDebug("Dragon HP: " + std::to_string(dragonHP));
    Engine::GetLogger().LogDebug("Fighter HP: " + std::to_string(fighterHP));
    Engine::GetLogger().LogDebug("Test spellslot: " + std::to_string(dragonslot));

    bool hasRogue = registry.HasKey("Rogue.max_hp");
