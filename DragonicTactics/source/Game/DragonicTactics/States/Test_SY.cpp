...
bool Test::Test_CombatSystem_CalculateDamage() {
    int damage = Engine::GetCombatSystem().CalculateDamage(dragon, fighter, "3d8", 5);
...
bool Test::Test_CombatSystem_CalculateDamage_MaxRoll() {
    int maxDamage = 0;
    for (int i = 0; i < 100; ++i) {
        int damage = Engine::GetCombatSystem().CalculateDamage(dragon, fighter, "3d8", 5);
...
bool Test::Test_CombatSystem_ApplyDamage() {
...
bool Test::Test_CombatSystem_ApplyDamage_Negative() {
    Dragon test_dragon({0,0});
    Fighter test_fighter({0,1});
...
bool Test::Test_CombatSystem_ExecuteAttack_Valid() {
    Dragon test_dragon({0,0});
    Fighter test_fighter({0,1});
...
bool Test::Test_CombatSystem_ExecuteAttack_OutOfRange() {
    Dragon test_dragon({0,0});
...
bool Test::Test_CombatSystem_ExecuteAttack_NotEnoughAP() {
    Dragon test_dragon({0,0});
    Fighter test_fighter({0, 1});
...
bool Test::Test_CombatSystem_IsInRange_Adjacent() {
    Dragon test_dragon({0,0});
    Fighter test_fighter({0, 1});
...
bool Test::Test_CombatSystem_IsInRange_TooFar() {
    Dragon test_dragon({0,0});
    Fighter test_fighter({5, 5});
    bool inRange = Engine::GetCombatSystem().IsInRange(&test_dragon, &test_fighter, 1);
...
bool Test::Test_CombatSystem_GetDistance() {
    Dragon test_dragon({0,0});
