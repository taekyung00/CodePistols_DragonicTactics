...
Character* CharacterFactory::Create(CharacterTypes type, Math::ivec2 start_position) {
    switch (type) {
        ...
        default:
            Engine::GetLogger().LogError("CharacterFactory::Create - Unknown character type: " 
...
Dragon* CharacterFactory::CreateDragon(Math::ivec2 position) {
    ...
    if (registry != nullptr) {
        ...
        Engine::GetLogger().LogDebug("CharacterFactory: Created Dragon from JSON at (" 
            + std::to_string(position.x) + ", " + std::to_string(position.y) 
            + ") - HP: " + std::to_string(data.max_hp) 
            + ", Speed: " + std::to_string(data.speed));
    } else {
        Engine::GetLogger().LogWarning("CharacterFactory: DataRegistry not found, using default Dragon stats");
    }
...
Fighter* CharacterFactory::CreateFighter(Math::ivec2 position) {
    ...
    if (registry != nullptr) {
        ...
        Engine::GetLogger().LogDebug("CharacterFactory: Created Fighter from JSON at (" 
            + std::to_string(position.x) + ", " + std::to_string(position.y) 
            + ") - HP: " + std::to_string(data.max_hp) 
            + ", Speed: " + std::to_string(data.speed));
    } else {
        Engine::GetLogger().LogWarning("CharacterFactory: DataRegistry not found, using default Fighter stats");
    }
