...
MovementComponent::MovementComponent(CS230::GameObject* owner) 
    : CS230::Component(), m_owner(owner) 
{
    if (m_owner != nullptr) {
...
void MovementComponent::Update(double dt) 
{
    ...
    if (m_moveTimer >= MOVE_TIME_PER_TILE) 
    {
        ...
        if (m_gridSystem->IsWalkable(next_pos)) 
        {
            ...
            m_owner->SetPosition({ static_cast<double>(next_pos.x * GridSystem::TILE_SIZE), 
