...
namespace Week3Tests
{
  ...
  void RunAllWeek3Tests()
  {
	...
	if (TestTreasureBoxCreation())
	  passed++;
	total++;
	if (TestTreasureBoxOpening())
	  passed++;
	total++;
	if (TestTreasureBoxVisualization())
	  passed++;
	total++;
	if (TestTreasureBoxGridInteraction())
	  passed++;
	total++;

	// Fireball tests
	std::cout << "\n--- FIREBALL SPELL TESTS ---" << std::endl;
	if (TestFireballDamage())
	  passed++;
	total++;
	if (TestFireballAreaOfEffect())
	  passed++;
	total++;
	if (TestFireballUpcasting())
	  passed++;
	total++;

	// Create Wall tests
	std::cout << "\n--- CREATE WALL SPELL TESTS ---" << std::endl;
	if (TestCreateWallPlacement())
	  passed++;
	total++;
	if (TestCreateWallBlocking())
	  passed++;
	total++;
	if (TestCreateWallUpcasting())
	  passed++;
	total++;

	// Lava Pool tests
	std::cout << "\n--- LAVA POOL SPELL TESTS ---" << std::endl;
	if (TestLavaPoolDamage())
	  passed++;
	total++;
	if (TestLavaPoolDuration())
	  passed++;
	total++;
	if (TestLavaPoolUpcasting())
	  passed++;
	total++;
