...
This folder is the actual game project: engine + gameplay source, assets, and build tooling.

| Path | Contents |
|---|---|
| `source/` | All C++ source. See [source/README.md](source/README.md) for the engine/game breakdown. |
| `Assets/` | Textures, sprites, animations, fonts, audio (BGM/SFX), and data files (`characters.json`, `maps.json`, `spell_table.csv`, `status_effect.csv`). |
| `cmake/` | CMake helper modules and third-party dependency fetch scripts. |
| `app_resources/` | Platform packaging resources (Windows icon/manifest, web `index.html` shell). |
| `scripts/` | Developer tooling — `scan_build_project.py` (build-matrix runner) and `make_release.py` (playtest ZIP + GitHub Release automation). |
| `docs/` | Local dev-environment setup and web-build debugging notes. |
| `build/` | CMake build output (git-ignored). |
| `CMakeLists.txt` / `CMakePresets.json` | Top-level build configuration and presets (`windows-debug`, `web-release`, etc.). |

For the system-by-system architecture (GameState/GameObject components, SpellSystem, AI strategies, etc.), see [CLAUDE.md](../CLAUDE.md) at the repository root.