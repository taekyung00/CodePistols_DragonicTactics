...
2025-10-15
머지후 그리드 시스템 수정
map으로 저장하던 캐릭터 위치 정보를 배열로 수정함
그리드를 컴포넌트로 선언
생성자 선언
Draw함수를 그림
addCharacter선언
moveCharacter선언
mock character를 그냥 캐릭터로 바꿈
모든 vec2를 ivec2로 변경
테스트 코드 수정
그리드 컴포넌트 넣고
스트링 벡터를 기반으로 맵 데이터 만듦
스트링 벡터 파싱 로직 만듦
draw를 통해서 시각화하고 방향키를 이용해 드래곤을 움직일수있게함
변수명 개선
...
2025-10-30
Input 시스템 리팩토링
Input::Update에서 매번 SDL 키보드 상태를 묻던(Polling) 방식을 제거
마우스 입력 기능추가
Engine 이벤트 시스템 구축
Engine::OnEvent 콜백 함수를 새로 만들어서, 모든 SDL 이벤트를 ImGui와 Input 시스템 양쪽으로 분배 하도록 함.
