...
def get_executable_info(folder, build_type, target):
    ...
    if target == "web":
        ...
        html_file = build_dir / "dragonic_tactics.html"
        if html_file.exists():
            return {
                'type': 'web',
                'path': str(html_file),
        ...
        exe_name = "dragonic_tactics.exe" if platform.system() == "Windows" else "dragonic_tactics"
        ...
        for result in successful_builds:
            ...
            if exe_info:
                ...
                if exe_info['type'] == 'web':
                    ...
                    colored_print(f"           # Then open: http://localhost:8000/dragonic_tactics.html", Colors.YELLOW)
                else:
