...
⚠️ **이 배포본은 git 워킹 트리가 아닐 수 있다** — zip 플레이테스트 패키지로 추출한 소스에는 어디에도 `.git`이 없어 모든 디렉토리에서 `git` 명령이 "not a git repository"로 실패한다. 버전 관리·`make_release.py --upload`의 `gh`/git 연동을 쓰려면 **정상 클론(git 루트가 존재하는 체크아웃)에서 작업**해야 하며, `make_release.py`는 `DragonicTactics/` 기준으로 실행된다. git 오류가 나면 먼저 `.git` 존재 여부(zip 배포본인지)부터 확인할 것.

ZIP 출력 위치: 저장소 루트(`DragonicTactics_vX.Y.Z_playtest.zip`) — 루트 `.gitignore`의 `*.zip` 규칙으로 자동 무시됨.
...
> 전체 원칙은 아래 두 가지 컴포넌트 시스템 규칙과 [architecture/architecture.md](architecture/architecture.md)를 참고.
...
- ⚠️ **호버 시 스텔스 알림은 제거됨** (`e9e04f4` "rouge cannot be attacked" alarm delete): `PlayerInputHandler::CheckStealthHoverNotice`와 `m_last_stealth_notice_tile_`는 삭제되었다. 스텔스 대상은 클릭 시 `CanCast`/타겟 선택이 조용히 실패할 뿐, 호버 중 `UINoticeEvent` 토스트는 더 이상 발행되지 않는다.
...
매 Update 프레임:
  ...
  4. EndTurn → TurnManager::EndCurrentTurn() (대기 없음)
     그 외  → AISystem::ExecuteDecision() 이 bool 반환:
              false(no-op: 도달 불가 경로 / CanCast 실패) → TurnManager::EndCurrentTurn() 즉시 호출 후 return (무한루프 최종 방지선)
              true → m_wait_timer 설정:
...
⚠️ **`AISystem::ExecuteDecision`은 `bool`을 반환한다** (실행 성공 여부). `Move`는 `FindPath` 결과가 빈 경로면 no-op, `UseAbility`는 `SpellSystem::CastSpell`의 `CanCast` 실패 시 no-op으로 간주해 `false`를 반환한다. `BattleOrchestrator`는 이 반환값이 `false`이면 즉시 `EndCurrentTurn()`을 호출한다 — Strategy별 개별 거리·스텔스 체크(위 Rogue/Cleric 무한루프 방지 항목들)를 놓치더라도 여기서 최종적으로 무한루프를 끊는 안전장치다.
...
god mode·`timeScale`(빨리감기)·각종 오버레이 토글을 보유한다. 위치: `source/Game/DragonicTactics/Debugger/` (`DebugManager` / `DebugConsole` / `DebugVisualizer`). 콘솔 명령어 전체 목록은 `DebugManager::RegisterGameCommands` 구현부 참고 (아래 목록은 주요 명령어만 발췌).
...
⚠️ `GamePlay.cpp`의 `SpellCastEvent` 구독 내부에 `std::cout` 디버그 출력이 남아 있다 — 릴리즈 전 제거 필요 (라인 459 근방, `[SOUND DEBUG]` 접두사로 검색).
...
- [architecture/architecture.md](architecture/architecture.md) — 시스템 아키텍처 개요
- [docs/turn-flow-chart.md](docs/turn-flow-chart.md) — 턴 진행 흐름 Mermaid 다이어그램
- [architecture/character_flowchart/](architecture/character_flowchart/) — 캐릭터별 AI 의사결정 Mermaid 플로우차트 (fighter/cleric/rouge/wizard)
...
- [docs/Detailed Implementations/features/cleric_implementation.md](docs/Detailed%20Implementations/features/cleric_implementation.md) — Cleric 구현 가이드
...
- ⚠️ `docs/Detailed Implementations/features/`에 `rogue_strategy.md`와 `rouge_strategy.md`가 모두 존재하던 시절이 있었다 — 구 철자 잔재였던 `rouge_strategy.md`는 삭제되었으므로 항상 `rogue_strategy.md`만 참조할 것
